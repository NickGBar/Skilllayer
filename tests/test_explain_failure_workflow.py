...
def test_schema_keys_identical_across_all_outcomes(
    all_pass_repo: Path,
    single_fail_repo: Path,
    no_runner_repo: Path,
    hang_repo: Path,
    garbled_output_repo: Path,
) -> None:
    cases = [
        SkillLayer().run(all_pass_repo, "explain why tests are failing")["artifacts"],
        SkillLayer().run(single_fail_repo, "explain why tests are failing")["artifacts"],
        SkillLayer().run(no_runner_repo, "explain why tests are failing")["artifacts"],
        SkillLayer(test_runner=TestRunner(timeout=1)).run(hang_repo, "explain why tests are failing")["artifacts"],
        SkillLayer().run(garbled_output_repo, "explain why tests are failing")["artifacts"],
