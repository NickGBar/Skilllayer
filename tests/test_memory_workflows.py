...
class TestClaimNextDecisionId:
    def test_first_id_is_0001(self, skilllayer_dir: Path) -> None:
        d = claim_next_id(skilllayer_dir, "next_decision_id")
        assert d == "0001"

    def test_second_id_is_0002(self, skilllayer_dir: Path) -> None:
        claim_next_id(skilllayer_dir, "next_decision_id")
        d = claim_next_id(skilllayer_dir, "next_decision_id")
        assert d == "0002"
    ...
    def test_corrupted_state_raises_and_preserves_original_file(self, skilllayer_dir: Path) -> None:
        # Milestone: Memory Storage Safety — corrupted .state.json must NEVER
        # be silently replaced with a reset counter (that risks minting a
        # duplicate id colliding with an already-existing decision/todo).
        # claim_next_id must raise, and the original corrupt bytes must
        # survive untouched as the only evidence of what went wrong.
        (skilllayer_dir / ".state.json").write_text("not json")
        with pytest.raises(MemoryStateCorruptError):
            claim_next_id(skilllayer_dir, "next_decision_id")
        assert (skilllayer_dir / ".state.json").read_text() == "not json"

    def test_independent_counter_keys_do_not_interfere(self, skilllayer_dir: Path) -> None:
        # The whole point of generalizing claim_next_decision_id into
        # claim_next_id(skilllayer_dir, counter_key): a second counter (e.g.
        # CrossSessionTodoWorkflow's "next_todo_id") must advance completely
        # independently of "next_decision_id" in the same .state.json.
        assert claim_next_id(skilllayer_dir, "next_decision_id") == "0001"
        assert claim_next_id(skilllayer_dir, "next_todo_id") == "0001"
        assert claim_next_id(skilllayer_dir, "next_decision_id") == "0002"
        assert claim_next_id(skilllayer_dir, "next_todo_id") == "0002"
        assert claim_next_id(skilllayer_dir, "next_decision_id") == "0003"
        state = json.loads((skilllayer_dir / ".state.json").read_text())
        assert state["next_decision_id"] == 4
...
class TestRehydrateContext:
    ...
    def test_no_memory_store_is_a_warning_not_an_error(self, tmp_path: Path) -> None:
        # A repo with no snapshot saved yet is a valid, usable empty state, not
        # a failure: success=True/status="warning"/error_code=None, with the
        # guidance surfaced through warnings instead of a hard error.
        d = tmp_path / ".skilllayer"
        d.mkdir()
        ...
        assert any("no .skilllayer" in w.lower() for w in result["warnings"])
...
class TestRegenerateIndex:
    ...
    def test_empty_store_has_no_needs_wrap_section(self, skilllayer_dir: Path) -> None:
        # No context snapshot and no file-watch data -> the NeedsWrap section is
        # not surfaced (matches how every other section appears only when its
        # source exists), so a brand-new store stays uncluttered.
        content = regenerate_index(skilllayer_dir)
        assert "NeedsWrap" not in content

    def test_needs_wrap_unknown_after_save_without_watch(self, skilllayer_dir: Path) -> None:
        # A context snapshot exists but WatchFileChanges has never run, so the
        # check cannot tell whether work happened — it must say "unknown", never
        # falsely claim "clean".
        save_context_snapshot(skilllayer_dir, state="S", open_questions=[])
        content = (skilllayer_dir / "INDEX.md").read_text()
        assert "## Unsaved Work · NeedsWrap — unknown" in content
        assert "clean" not in content.lower()
...
class TestErrorCodeMessages:
    ...
    def test_no_memory_store_has_exact_message(self) -> None:
        ...
        assert "No .skilllayer/ memory store found" in msg
        assert "skilllayer_save_context" in msg
        assert "MCP" in msg
    ...
    def test_no_memory_store_is_not_circular(self) -> None:
        # Regression test: the old message told the user to run 'Save context
        # snapshot' — a memory *write* workflow that the CLI itself refuses to
        # run (it's MCP-only, see TestWriteWorkflowsUnsupportedViaCLI in
        # test_memory_cli_dispatch.py). That's a dead end: the CLI's own error
        # told the user to run a command the CLI's own dispatcher rejects.
        # The fixed message must name the actual MCP tool and explain why the
        # CLI can't do it itself, not just repeat the bare workflow name as if
        # it were a runnable CLI command.
        msg = _ERROR_CODE_MESSAGES["no_memory_store"]
        assert "skilllayer_save_context" in msg
        assert "cannot" in msg.lower()
        assert "docs/CLAUDE_CODE_SETUP.md" in msg
...
class TestPrintHumanRunMemoryWorkflows:
    ...
    def test_rehydrate_no_memory_store_prints_error_message(self) -> None:
        ...
        output = buf.getvalue()
        assert "No .skilllayer/ memory store found" in output, (
            f"Expected no_memory_store human message in output:\n{output}"
        )
        assert "skilllayer_save_context" in output, (
            f"Expected the MCP tool name in output:\n{output}"
        )
