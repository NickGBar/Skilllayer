...
DEFAULT_CONFIG = {
    ...
    "logging": {
        # Run logs are opt-in. A read-only workflow must not create repository-
        # local artifacts merely because the caller used the default config.
        "log_dir": None,
    },
    "browser_smoke": {
        "url": None,
        ...
        # Screenshots/reports are useful but are persistent writes. Require an
        # explicit config opt-in instead of producing them during every smoke.
