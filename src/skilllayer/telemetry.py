...
AUTOMATIC_TELEMETRY_ENV = "SKILLLAYER_TELEMETRY_ENABLED"
...
def automatic_telemetry_enabled() -> bool:
    """Return whether cross-cutting CLI/MCP/workflow recording was opted into.

    Direct telemetry/report APIs remain available. This gate only prevents an
    otherwise read-only command or workflow from creating runs/ artifacts as a
    hidden side effect.
    """
    return os.environ.get(AUTOMATIC_TELEMETRY_ENV, "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def automatic_telemetry_paths() -> list[str]:
    return [
        str(TELEMETRY_JSONL),
        str(CLI_ACTIVITY_JSONL),
        str(TELEMETRY_SUMMARY),
    ]


def tool_telemetry_paths() -> list[str]:
    """Exact files written by one opted-in tool/MCP invocation."""
    return [str(TELEMETRY_JSONL), str(TELEMETRY_SUMMARY)]


def cli_activity_paths() -> list[str]:
    """Exact files written by one opted-in CLI activity event."""
    return [str(CLI_ACTIVITY_JSONL), str(TELEMETRY_SUMMARY)]
