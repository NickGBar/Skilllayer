...
class SkillLayer:
    ...
    def run(
        ...
        *,
        ...
        tools = ProjectTools(repo, log=log, test_runner=self.test_runner)
        ...
        if route.task_type == "browser_smoke":
            result.update(
                {
                    "console_errors": int(browser_smoke_result.get("console_errors", 0)),
                    "network_errors": int(browser_smoke_result.get("network_errors", 0)),
        ...
        self._record_demand_event_best_effort(route, task_description, result, log, bool(macro_sequence))
        self._record_usage_event_best_effort(route.workflow, task_description, started, route.llm_calls, result)
        return result
    ...
    def _record_demand_event_best_effort(
        ...
        pass  # demand tracking removed for public release
    ...
    def _record_usage_event_best_effort(
        ...
        pass  # cost tracking removed for public release
...
def outcome_reason_for_run_tests_outcome(outcome: str) -> str:
    reasons = {
        ...
        "TIMEOUT": "The test command exceeded the configured timeout.",
...
def workflow_execution_success_for_run_tests_outcome(outcome: str) -> bool:
    return outcome in {"PASSED", "FAILED_TESTS", "NO_TESTS_DISCOVERED", "NO_TEST_COMMAND", "UNSUPPORTED"}
...
def extract_test_summary(test_result: dict[str, Any], *, tests_run: bool, tests_passed: bool | None, failure_count: int) -> str:
    ...
    if bool(test_result.get("timed_out")):
        return "Test command timed out before completion."
    if test_result_no_tests_discovered(test_result):
...
def build_monitor_flakiness_artifacts(
    ...
    cmd = list(base_cmd)
    if test_identifier and test_identifier != ".":
        cmd.append(test_identifier)
...
def _ensure_baseline_gitignored(repo: Path) -> None:
    entry = ".skilllayer/test_speed_baseline.json"
...
def build_measure_test_speed_artifacts(repo: Path) -> dict[str, Any]:
    ...
    try:
        ...
        baseline_path.write_text(
            _json.dumps({"duration_ms": total_duration_ms, "checked_at": checked_at, "test_count": test_count}, indent=2),
            "utf-8",
        )
    except OSError:
        pass

    _ensure_baseline_gitignored(repo)
...
def _fetch_pypi_latest(package: str) -> str | None:
    """Fetch latest version from PyPI JSON API. Returns None on any failure."""
...
def _fetch_npm_latest(package: str) -> str | None:
    """Fetch latest version from npm registry. Returns None on any failure."""
    import urllib.request
    import urllib.error
...
def build_watch_deps_artifacts(repo: Path) -> dict[str, Any]:
    ...
    up_to_date_count = sum(1 for d in dependencies if not d["outdated"] and d["latest_version"] is not None)
    unknown_count = sum(1 for d in dependencies if d["latest_version"] is None)
...
def _ensure_activity_snapshot_gitignored(repo: Path) -> None:
    entry = ".skilllayer/repo_activity_snapshot.json"
    gitignore = repo / ".gitignore"
    try:
        if gitignore.exists():
            content = gitignore.read_text("utf-8")
...
def build_detect_activity_artifacts(repo: Path) -> dict[str, Any]:
    ...
    try:
        ...
        snapshot_path.write_text(_json.dumps(new_snapshot, indent=2), "utf-8")
    except OSError:
        pass

    _ensure_activity_snapshot_gitignored(repo)
...
def _ensure_profile_baseline_gitignored(repo: Path) -> None:
    entry = ".skilllayer/profile_baseline.json"
    gitignore = repo / ".gitignore"
    try:
        if gitignore.exists():
            content = gitignore.read_text("utf-8")
    ...
    *,
    ...
    if not target_path.is_file():
        return _build_profile_error(
            "file_not_found", f"File not found: {target}", target, checked_at
        )
    ...
    try:
        ...
        return _build_profile_error(
            "execution_error", str(exc), target, checked_at
        )
    ...
    _ensure_profile_baseline_gitignored(repo)
...
def _ensure_memory_baseline_gitignored(repo: Path) -> None:
    entry = ".skilllayer/memory_baseline.json"
    gitignore = repo / ".gitignore"
    try:
        if gitignore.exists():
            content = gitignore.read_text("utf-8")
...
def build_measure_memory_artifacts(repo: Path, target: str) -> dict[str, Any]:
    ...
    _ensure_memory_baseline_gitignored(repo)
...
_SECRET_PATTERNS: list[dict[str, Any]] = [
    ...
    {"name": "aws_secret_key", "severity": "critical",
     "pattern": re.compile(r"aws_secret\w*\s*=\s*[a-zA-Z0-9/+=]{40}", re.I)},
    ...
    {"name": "generic_api_key", "severity": "high",
     "pattern": re.compile(r"""(?:api_key|apikey|api_secret)\s*=\s*["'][^"']{16,}["']""", re.I)},
    {"name": "generic_token", "severity": "high",
     "pattern": re.compile(r"""\b(?:token|secret|password)\s*=\s*["'][^"']{16,}["']""", re.I)},
...
def build_check_port_artifacts(port: int, host: str = "127.0.0.1") -> dict[str, Any]:
    ...
    # Probe availability via a non-blocking connect attempt.
...
def build_save_context_artifacts(
    ...
    return save_context_snapshot(repo / ".skilllayer", state, open_questions)
...
def build_remember_preferences_artifacts(
    ...
    return remember_preferences(repo / ".skilllayer", preferences)
...
def flatten_workflow_artifacts(workflow: str, artifacts: dict[str, Any]) -> dict[str, Any]:
    ...
    if workflow == "DetectSecretPatternsWorkflow":
        return {
            ...
            "clean": artifacts.get("clean", True),
            "checked_at": artifacts.get("checked_at"),
        }
