...
if command -v python3 &>/dev/null; then
  if ! python3 -c 'import sys; raise SystemExit(0 if sys.version_info >= (3, 10) else 1)' 2>/dev/null; then
    ...
    echo "    python -m pip install -e '.[mcp]' --no-build-isolation" >&2
    exit 1
  fi
fi
...
echo "Installing SkillLayer in editable mode with MCP extra"
if ! "${PYTHON}" -m pip install -e ".[mcp]" --no-build-isolation; then
  echo "warning: MCP extra install failed; falling back to CLI-only install." >&2
  if ! "${PYTHON}" -m pip install -e . --no-build-isolation; then
    echo "INSTALL FAILED" >&2
    echo "Reason: both MCP-extra and CLI-only installs failed." >&2
    exit 1
  fi
  echo "warning: MCP clients may need: ${PYTHON} -m pip install -e \".[mcp]\" --no-build-isolation" >&2
fi

echo "Running tester-check"
if ! "${PYTHON}" -m skilllayer tester-check; then
  echo "INSTALL FAILED" >&2
  echo "Reason: tester-check failed after installation." >&2
  exit 1
fi
...
echo "  ${PYTHON} -m skilllayer inspect --repo /path/to/repo --json"
echo "  ${PYTHON} scripts/generate_mcp_config.py"
