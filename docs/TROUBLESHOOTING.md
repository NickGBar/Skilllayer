...
## Telemetry Export Privacy Scan Fails

The export command blocks unsafe output by default.

Run:

```bash
python -m skilllayer telemetry-export --json
```

Review the privacy scan findings. Do not use `--allow-unsafe` unless you have
manually reviewed the output and are certain it contains nothing private.
