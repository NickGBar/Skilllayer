...
## Step 3: Run Inspection and Test Workflows
...
Run tests only on a clean branch or copy; repository-defined tests may create
caches, reports, databases, or external side effects:
...
## Step 6: Do Not Share Telemetry Automatically

Telemetry is local by default. Anonymous export is opt-in only:

```bash
python -m skilllayer telemetry-export
```

Review the export before sharing. Do not share raw logs, private code, diffs,
screenshots, credentials, or local paths.
